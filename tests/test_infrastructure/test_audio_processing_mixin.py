...
import numpy as np
from audio_transcribing.infrastructure.audio_processing import AudioProcessing
...
class TestAudioProcessingMixin(unittest.TestCase):
    def test_get_audio_stream_valid_audio(self):
        ...
        with self.assertRaises(Exception):
            AudioProcessing.get_audio_stream(content)

    def test_get_mono_audio_already_mono(self):
        audio = np.array([0.1, 0.2, 0.3])
        result = AudioProcessing.get_mono_audio(audio)
    ...
    def test_resample_audio_16k_sample_rate(self):
        ...
        sample_rate = 16000
        result = AudioProcessing.resample_audio(audio, sample_rate)
        self.assertTrue(np.array_equal(result, audio))
